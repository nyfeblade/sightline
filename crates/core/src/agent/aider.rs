...
//! on. So conversations are found by looking where Sightline has seen Aider
...
//! is where Sightline's figures for an Aider session come from.
...
impl Adapter for Aider {
    ...
    fn command(&self, options: Options) -> Vec<String> {
        ...
        // Aider has a model and nothing else Sightline offers; effort and
...
/// Sightline follow a session as it works rather than re-reading the file.
